...
\caption{Umformung der Knoten mit MapReduce
    ...
    \begin{algorithmic}[1]
        \Function{map}{node}
            \ForAll{node.documents}
                \State \Call{Emit}{document, \{terms: [node]\}}
            \EndFor
        \EndFunction
        \Statex
        \Function{reduce}{term, values}
            \State $result := \{terms: []\}$
            \ForAll{values}
                \State $result.terms :=$ \Call{concat}{result.terms, value.terms}
            \EndFor
            \State \Return{result}
        \EndFunction
        \Statex
...
In Algorithmus \ref{alg:edges2} wird die Erzeugung der Kookkurenzkanten dargestellt. Im Map-Schritt werden dazu alle möglichen Paare der mit einem Dokument verknüpften Begriffe gebildet und emittiert. Der Schlüssel ist dabei eine Kombination aus Ziel- und Quellbegriff. Der Wert zählt die Anzahl der Kookkurenzen zwischen beiden Begriffen. Im Reduce-Schritt werden alle Kanten zwischen zwei Termen zusammengefasst, die Summe der Kookkurenzen gebildet und die Ähnlichkeitsmaße berechnet. Die Funktionen zur Berechnung der Maße sind in \ref{measures} beschrieben.

\begin{algorithm}
\caption{Kantenerzeugung mit MapReduce
    \label{alg:edges2}}
    \begin{algorithmic}[1]
        \Function{map}{document}
            \ForAll{term1 in document.terms}
                \ForAll{term2 in document.terms}
                    \State \Call{Emit}{\{source: term1, target: term2\}, \{count: 1\}}
                \EndFor
            \EndFor
        \EndFunction
        \Statex
        \Function{reduce}{edge, values}
            \State $result := \{count: 0, dice: 0, jaccard: 0, cosine: 0\}$
            \ForAll{values}
                \State $result.count := result.count + value.count$
            \EndFor
            \State $result.dice = $ \Call{dice}{edge.source, edge.target, result.count}
            \State $result.jaccard = $ \Call{jaccard}{edge.source, edge.target, result.count}
            \State $result.cosine = $ \Call{cosine}{edge.source, edge.target, result.count}
            \State \Return{result}
        \EndFunction
    \end{algorithmic}
\end{algorithm}
